...
def affect_analysis(cadena, agresor_array):
	total_words = 0
...
def affect_processing(archivo):
	agresor_array = []
	count_frases = 0
	with open(archivo) as archivo: # Abre el archivo en una variable nombrada "archivo"
		for frase in archivo: # recorre cada frase del archivo individualmente
			...
			total_words = affect_analysis(frase_split, agresor_array) # indica a qué afecto corresponde cada palabra restante
	...
	if agresor_detect[-1:] == ':':
		agresor_detect = agresor_detect[:-1].capitalize()
	else:
		agresor_detect = agresor_detect.capitalize()

	if victima_detect[-1:] == ':':
		victima_detect = victima_detect[:-1].capitalize()
	else:
		victima_detect = victima_detect.capitalize()
